...
func _ready():
	...
	spawn_point.connect("is_depleted", self, "_on_SpawnPoint_is_depleted")
...
func _get_configuration_warning():
	if not has_valid_spawn_point():
		return "Parent must be a SpawnPoint"
	return ""

func has_valid_spawn_point() -> bool:
	var p = get_parent()
	return p is SpawnPoint || not spawn_point == null
...
func _on_SpawnPoint_is_depleted(who):
	if who != spawn_point:
		return
